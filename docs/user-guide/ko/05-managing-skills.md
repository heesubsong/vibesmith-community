...
**스킬(Skills)**은 AI 에이전트가 특정 작업을 수행할 수 있도록 하는 재사용 가능한 명령어 세트입니다.

### 스킬의 장점

- **재사용성**: 한 번 작성하면 여러 프로젝트에서 사용
- **일관성**: 팀 전체가 동일한 패턴으로 작업
- **자동화**: 반복적인 작업을 자동화
- **공유**: 커뮤니티와 베스트 프랙티스 공유
...
![스킬 페이지로 이동](images/skills-01-list.png)

왼쪽 사이드바에서 **Skills** 메뉴를 클릭하면 모든 스킬 목록을 볼 수 있습니다.

### 스킬 리스트 정보

각 스킬 카드에는 다음 정보가 표시됩니다:

- **이름**: 스킬의 고유 식별자 (kebab-case)
- **설명**: 스킬이 하는 일에 대한 간단한 설명
- **태그**: 카테고리 및 용도 분류
  - `code-gen`: 코드 생성
  - `refactoring`: 리팩토링
  - `testing`: 테스트
...
- **카드 뷰**: 시각적으로 풍부한 카드 형태 (기본)
- **리스트 뷰**: 간결한 리스트 형태
- **그리드 뷰**: 더 많은 스킬을 한 번에 보기

### 💡 유용한 팁

💡 검색창을 사용하면 원하는 스킬을 빠르게 찾을 수 있습니다.
💡 태그를 클릭하면 해당 태그의 스킬만 필터링됩니다.
💡 리스트는 이름, 최근 수정일, 사용 빈도, 카테고리 기준으로 정렬할 수 있습니다.
💡 즐겨찾기(⭐)를 설정하면 자주 사용하는 스킬에 빠르게 접근할 수 있습니다.
